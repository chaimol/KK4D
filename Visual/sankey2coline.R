...
if (length(args) < 6){
  ...
  {
    if (TRUE) {stop("please input 6 parameters")}
...
#要求输入的gff3的顺序和最后面的拉丁名的顺序一样，而且共线性文件列的顺序也应该是coline:物种1，物种2
gff3file1 <- args[1] #物种1的gff3文件
gff3file2 <- args[2] #物种2的gff3文件
colinefile <- args[3] #两列基因ID的共线性文件
latin_label <- c(args[4],args[5]) #物种的拉丁名
workpath <- args[6]
setwd(workpath)
...
coline_gene <- read.delim(colinefile,header = F)
colnames(coline_gene) <- c("geneID1","geneID2")
data_all <- inner_join(inner_join(coline_gene,gff3_1,by="geneID1"),gff3_2,by="geneID2")

dat_all <- gather(data_all,key="geneID",value="GeneID",c(geneID1,geneID2)) %>% 
...
ggsave(paste0(latin_label[1],".",latin_label[2],".sankey.png"),p1/p2)
message("Output image in ",getwd(),"/",latin_label[1],".",latin_label[2],".sankey.pdf")
